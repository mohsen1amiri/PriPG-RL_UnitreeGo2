...
# 2. ENVIRONMENT CLASS
# =====================================================================
class Go2IsaacLabNavEnv_v1(gym.Env):
    ...
    self.use_3d_action = use_3d_action
    ...
    self.ablation_beta = ablation_beta
    ...
    def step(self, action):
        ...
        if self.planner is not None:
            ...
            # if self.use_3d_action:
            #     L, rho = 0.05, 0.0
            #     V = np.hypot(u_yw, -u_xw)
            #     e_t = np.arctan2(-u_xw, u_yw) - robot_yaw_t
            #     e_t = (e_t + np.pi) % (2 * np.pi) - np.pi
            #     exp_vx = V * np.cos(e_t)
            #     exp_vy = (1.0 - rho) * V * np.sin(e_t)
            #     exp_yaw = rho * (V * np.sin(e_t)) / L
            #     expert_action_array = np.array([np.clip(exp_vx, -2.0, 2.0), np.clip(exp_vy, -2.0, 2.0), np.clip(exp_yaw, -2.0, 2.0)], dtype=np.float32)
            if self.use_3d_action:
                L, rho = 0.3, 0.99  # Note: rho=0.0 means ZERO turning (pure crab-walk)
                
                # 1. Get magnitude and true GLOBAL target angle
                V = np.hypot(u_xw, u_yw)
                target_yaw = np.arctan2(u_yw, u_xw) 
                
                # 2. Calculate local heading error (Target Angle - Robot Angle)
                e_t = target_yaw - robot_yaw_t
                e_t = (e_t + np.pi) % (2 * np.pi) - np.pi
                
                # 3. Apply Unicycle kinematics based on the error
                exp_vx = V * np.cos(e_t)
                exp_vy = (1.0 - rho) * V * np.sin(e_t)
                exp_yaw = rho * (V * np.sin(e_t)) / L
            ...
            #     V = np.hypot(u_xw, u_yw)                  # Total desired speed
            #     target_yaw = np.arctan2(-u_xw, u_yw)       # Desired global heading
                
            #     # Calculate heading error and wrap it to [-pi, pi]
            #     yaw_error = target_yaw - robot_yaw_t
            #     yaw_error = (yaw_error + np.pi) % (2 * np.pi) - np.pi
                
            #     k_p = 10.0  # Proportional gain for turning (Tune this: higher = snappier turns)
                
            #     exp_vx = V * np.cos(yaw_error)   # Forward speed (slows down if facing the wrong way)
            #     exp_vy = 0.0                     # Strictly zero crab-walking
            #     exp_yaw = k_p * yaw_error        # Turn rate proportional to error
            ...
            #     ], dtype=np.float32)
                ...
                expert_action_array = np.array([np.clip(u_yw, -2.0, 2.0), np.clip(-u_xw, -2.0, 2.0)], dtype=np.float32)

        # # Convex Combination
        # if self.ablation_beta == "beta_1":
        #     beta_t = 1.0
        # elif self.ablation_beta == "beta_0":
        #     beta_t = 0.0
        # else:
        #     if self.M_t == 1.0:
        #         beta_t = 0.0 
        #     else:
        #         progress = min(1.0, float(self.total_t) / float(self.burn_in_steps))
        #         beta_t = 1.0 - (1.0 - self.beta_min) * progress
            
        # if expert_action_array is not None:
        #     expert_tensor = torch.tensor(expert_action_array, device=self.device, dtype=torch.float32)
        #     executed_action_tensor = (beta_t * expert_tensor) + ((1.0 - beta_t) * act_tensor)
        # else:
        #     executed_action_tensor = act_tensor

        executed_action_tensor = act_tensor

        self.current_obs_dict["policy"][:, 9] = executed_action_tensor[0]   
        self.current_obs_dict["policy"][:, 10] = executed_action_tensor[1]  
        
        if self.use_3d_action:
            self.current_obs_dict["policy"][:, 11] = executed_action_tensor[2] 
        else:
            self.current_obs_dict["policy"][:, 11] = 0.0

        # ==========================================================
        # RESTORED: SYNC THE GREEN ARROW (Visuals in Isaac Lab)
        # ==========================================================
        vel_command = self.base_env.unwrapped.command_manager.get_command("base_velocity")
        vel_command[:, 0] = executed_action_tensor[0] 
        vel_command[:, 1] = executed_action_tensor[1]  
        vel_command[:, 2] = executed_action_tensor[2] if self.use_3d_action else 0.0
        # ==========================================================
        ...
        if terminated or truncated:
            ...
            if info.get("is_success", 0.0) == 1.0:
                ...
                # --- SURGICAL FIX: Using step_dt instead of sim.dt ---
