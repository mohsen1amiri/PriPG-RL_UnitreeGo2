...
# --- CORE ARGS ---
parser.add_argument("--algo", type=str, default="compass", choices=["compass", "sac", "ppo", "accel_sac", "expert"], help="Which algorithm to train.")
...
# --- COMPASS RL ARGS ---
parser.add_argument("--anchor_weight", type=float, default=1.0, help="Weight of the Forward KL (NLL) anchor.")
parser.add_argument("--gate_tau", type=float, default=1.0, help="Temperature for the Q-Filter sigmoid.")
parser.add_argument("--stationarity_tolerance", type=float, default=1e-5, help="xi: Microscopic slope tolerance.")
parser.add_argument("--patience_window", type=int, default=5000, help="K: Steps required to prove mastery.")
parser.add_argument("--confidence_delta", type=float, default=0.05, help="Delta: Confidence interval.")
parser.add_argument("--loss_range_b", type=float, default=4.0, help="Max theoretical MSE range.")
...
# --- ENVIRONMENT ARGS ---
parser.add_argument("--use_3d_action", action="store_true", default=False, help="Enable 3D action space (vx, vy, yaw's rate) vs 2D (vx, vy).")
...
parser.add_argument("--burn_in_steps", type=int, default=100_000, help="T_burn: Strict kinematic burn-in horizon.")
parser.add_argument("--beta_min", type=float, default=0.9, help="Minimum expert control authority during burn-in.")

# --- ABLATION ARGS ---
parser.add_argument("--ablation_maturation", type=str, default="none", choices=["none", "naive_decay"], help="Ablate UCB maturation.")
parser.add_argument("--ablation_beta", type=str, default="none", choices=["none", "beta_1", "beta_0"], help="Ablate data collection beta.")
...
import torch
import numpy as np
from go2_env_v1 import Go2IsaacLabNavEnv
...
def main():
    ...
    if args_cli.algo in ["sac", "ppo"]:
        ...
        args_cli.ablation_beta = "beta_0" # RL Agent strictly controls hardware
        ...
        args_cli.ablation_beta = "beta_1" # Expert 100% in control
        # Planner MUST remain active.
    
    train_env = Go2IsaacLabNavEnv(
        planner=planner, 
        ...
        burn_in_steps=args_cli.burn_in_steps,
        beta_min=args_cli.beta_min,
        ablation_beta=args_cli.ablation_beta,
        use_3d_action=args_cli.use_3d_action 
    )
    ...
    if args_cli.algo == "compass":
        model = COMPASS(
            ...
            anchor_weight=args_cli.anchor_weight,
            gate_tau=args_cli.gate_tau,
            stationarity_tolerance=args_cli.stationarity_tolerance,
            patience_window=args_cli.patience_window,
            confidence_delta=args_cli.confidence_delta,
            loss_range_b=args_cli.loss_range_b,
            burn_in_steps=args_cli.burn_in_steps,
            ablation_maturation=args_cli.ablation_maturation
        )
