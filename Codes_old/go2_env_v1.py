...
# Isaac Sim imports used inside the env
from isaacsim.core.api import World
from isaacsim.core.prims import SingleArticulation
from isaacsim.core.utils.stage import add_reference_to_stage
from isaacsim.core.utils.nucleus import get_assets_root_path
from isaacsim.core.api.objects import FixedCylinder, VisualSphere, FixedCuboid
from isaacsim.core.simulation_manager import SimulationManager
from isaacsim.core.utils.rotations import quat_to_rot_matrix, quat_to_euler_angles, euler_angles_to_quat
...
class Go2REAPEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.world = World(
            backend="numpy",
            set_defaults=False,          # IMPORTANT: don't silently force cm + 1/60
            stage_units_in_meters=1.0,   # IMPORTANT: make your numbers meters
            physics_dt=self.DT_SIM,
            rendering_dt=self.DT_SIM,
        )
        print(">> [DIAG] physics_dt =", SimulationManager.get_physics_dt())

        try:
            self.world.set_simulation_dt(physics_dt=self.DT_SIM, rendering_dt=self.DT_SIM)
            print(f">> [Env] Set world dt via set_simulation_dt: {self.DT_SIM}")
        except Exception:
            try:
                phys = self.world.get_physics_context()
                phys.set_time_step(self.DT_SIM)
                print(f">> [Env] Set physics dt via physics_context.set_time_step: {self.DT_SIM}")
            except Exception:
        ...
        print(">> [STAGE] MetersPerUnit:", UsdGeom.GetStageMetersPerUnit(stage))
        ...
        # ----------------------------
        # Sanity checks (timing / lockstep)
        # ----------------------------
        self.debug_lockstep_check = True     # set False to disable
        self._did_lockstep_check = False
        self._lockstep_tol = 1e-4            # position tolerance

        self._v_f = np.zeros(2, dtype=np.float32)
        self._tau_cmd = 0.2  # seconds

        # ---- Run lockstep sanity check ONCE at env construction ----
        self._sanity_check_lockstep()
    ...
    def _sanity_check_lockstep(self):
        """
        DIAGNOSTIC ONLY:
        Prints evidence to decide which hypothesis is correct.
        Does NOT raise / crash the job.
        """
        if (not self.debug_lockstep_check) or self._did_lockstep_check:
            return
        self._did_lockstep_check = True

        print("\n================ [SANITY DIAG START] ================", flush=True)

        # ---- Stage settings check (hypothesis #3) ----
        stage = omni.usd.get_context().get_stage()
        print(">> [STAGE] UpAxis:", UsdGeom.GetStageUpAxis(stage), flush=True)
        print(">> [STAGE] MetersPerUnit:", UsdGeom.GetStageMetersPerUnit(stage), flush=True)

        # ---- dt check (can explain your 0.0055 displacement) ----
        try:
            print(">> [DT] SimulationManager.get_physics_dt():", SimulationManager.get_physics_dt(), flush=True)
        except Exception as e:
            print(">> [DT] SimulationManager.get_physics_dt() failed:", e, flush=True)

        try:
            phys = self.world.get_physics_context()
            # Some versions have get_time_step()
            if hasattr(phys, "get_time_step"):
                print(">> [DT] physics_context.get_time_step():", phys.get_time_step(), flush=True)
        except Exception as e:
            print(">> [DT] physics_context dt read failed:", e, flush=True)

        # ---- Settle ----
        self.robot.set_linear_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
        self.robot.set_angular_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
        for _ in range(5000):
            self.world.step(render=False)

        # ---- Pose + orientation check (hypothesis #2) ----
        pos0, quat0 = self.robot.get_world_pose()
        obs0 = self._get_obs().copy()

        print(">> [POSE] pos0:", pos0, flush=True)
        print(">> [POSE] quat0:", quat0, flush=True)
        print(">> [POSE] obs0:", obs0, flush=True)

        # Convert quat -> yaw and body axes in world
        try:
            R = quat_to_rot_matrix(quat0)  # 3x3
            ex_w = R @ np.array([1.0, 0.0, 0.0])  # body +x expressed in world
            ey_w = R @ np.array([0.0, 1.0, 0.0])  # body +y expressed in world
            eul = quat_to_euler_angles(quat0)     # typically [roll, pitch, yaw] in radians
            print(">> [ORIENT] euler(r,p,y):", eul, flush=True)
            print(">> [ORIENT] body +x in world:", ex_w, flush=True)
            print(">> [ORIENT] body +y in world:", ey_w, flush=True)
        except Exception as e:
            print(">> [ORIENT] quat->axes failed:", e, flush=True)

        # ---- Drift/settling check (hypothesis #4) ----
        # If obs moves significantly with zero velocity, contacts/settling dominate.
        self.robot.set_linear_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
        for _ in range(5000):
            self.world.step(render=False)
        obs_drift = self._get_obs().copy()
        print(">> [DRIFT] obs after zero-vel steps:", obs_drift, " delta:", (obs_drift - obs0), flush=True)

        # ---- Core mapping test (hypothesis #1 vs #2) ----
        # Command +x then +y and measure displacement in OBS frame
        # Step A: command +x
        self.robot.set_linear_velocity(np.array([1.0, 0.0, 0.0], dtype=np.float32))
        for _ in range(5000):
            self.world.step(render=False)
        obs1 = self._get_obs().copy()

        # Step B: command +y
        self.robot.set_linear_velocity(np.array([0.0, 1.0, 0.0], dtype=np.float32))
        for _ in range(5000):
            self.world.step(render=False)
        obs2 = self._get_obs().copy()

        d1 = obs1 - obs0
        d2 = obs2 - obs1
        print(">> [MAP] d1 (after cmd +x):", d1, flush=True)
        print(">> [MAP] d2 (after cmd +y):", d2, flush=True)

        # ---- Yaw-dependence test (this decides body-frame vs world-frame definitively) ----
        # Rotate robot +90deg yaw and repeat the same commands.
        # If velocity is interpreted in BODY frame, the mapping rotates with yaw.
        # If velocity is interpreted in WORLD frame, mapping stays the same.
        try:
            yaw90 = euler_angles_to_quat(np.array([0.0, 0.0, np.pi / 2], dtype=np.float32))
            self.robot.set_world_pose(position=pos0, orientation=yaw90)
            self.robot.set_linear_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
            self.robot.set_angular_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
            for _ in range(5000):
                self.world.step(render=False)

            obs0r = self._get_obs().copy()

            self.robot.set_linear_velocity(np.array([1.0, 0.0, 0.0], dtype=np.float32))
            for _ in range(5000):
                self.world.step(render=False)
            obs1r = self._get_obs().copy()

            self.robot.set_linear_velocity(np.array([0.0, 1.0, 0.0], dtype=np.float32))
            for _ in range(5000):
                self.world.step(render=False)
            obs2r = self._get_obs().copy()

            d1r = obs1r - obs0r
            d2r = obs2r - obs1r
            print(">> [YAWTEST] After +90deg yaw, d1r (cmd +x):", d1r, flush=True)
            print(">> [YAWTEST] After +90deg yaw, d2r (cmd +y):", d2r, flush=True)
        except Exception as e:
            print(">> [YAWTEST] failed:", e, flush=True)

        # Restore original pose
        self.robot.set_world_pose(position=pos0, orientation=quat0)
        self.robot.set_linear_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
        self.robot.set_angular_velocity(np.array([0.0, 0.0, 0.0], dtype=np.float32))
        for _ in range(5000):
            self.world.step(render=False)

        print("================= [SANITY DIAG END] =================\n", flush=True)
    ...
    def step(self, action):
        ...
        vx, vy = np.clip(action, [-2.5, -1.0], [3.8, 1.0])  # match Sport ranges :contentReference[oaicite:13]{index=13}
        alpha = self.DT_SIM / max(self._tau_cmd, 1e-6)
        alpha = np.clip(alpha, 0.0, 1.0)
        self._v_f[0] += alpha * (vx - self._v_f[0])
        self._v_f[1] += alpha * (vy - self._v_f[1])
        velocity_command = np.array([self._v_f[0], self._v_f[1], 0.0])
