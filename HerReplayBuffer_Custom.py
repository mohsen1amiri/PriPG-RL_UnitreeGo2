...
from type_aliases_custom import CustomDictReplayBufferSamples
...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        goal_selection_strategy: Union[GoalSelectionStrategy, str] = "episode",
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> CustomDictReplayBufferSamples:  # type: ignore[override]
        """
        ...
        # **NEW**: Concatenate the infos lists (simply join the lists)
        infos = {}
        for key in real_data.infos.keys():
            real_vals = real_data.infos[key]
            virtual_vals = virtual_data.infos[key]
            # Check the type of the first element to decide how to merge
            if isinstance(real_vals[0], th.Tensor):
                # Both values are expected to be tensors, so we can concatenate them
                infos[key] = th.cat((real_vals, virtual_vals), dim=0)
            else:
                # For non-tensor data (e.g., strings, numbers, or other objects), we can simply concatenate the lists
                infos[key] = real_vals + virtual_vals
    ...
    def _get_real_samples(
        ...
        return CustomDictReplayBufferSamples(
            ...
            infos=self.infos[batch_indices, env_indices],  # <-- Newly added field
        )
